...
export default function HomePage() {
  ...
  ) { set({t,s}); setTimeout(()=>set(null),4000); }
  ...
  async function handleGoogle() {
    showToast("🔑","Redirecting to Google OAuth..."); await signIn("google");
  }
  async function handleSignIn() {
    ...
    if (res?.error) flash(setSigninMsg,"error","Invalid email or password.");
  }
  async function handleSignUp() {
    ...
    if (signupPass.length<8){flash(setSignupMsg,"error","Password >= 8 characters.");return;}
    setSignupLoading(true);
    const otp = Math.floor(100000+Math.random()*900000).toString();
    setPendingEmail(signupEmail); setGenOTP(otp);
    setSignupLoading(false); setAuthTab("otp"); setOtpDigits(["","","","","",""]);
    showToast("📧",`Code sent to ${signupEmail} (Demo: ${otp})`);
  ...
  async function handleVerifyOTP(){
    ...
    setOtpLoading(false);
    if(entered===genOTP){
      flash(setOtpMsg,"success","Verified! Signing you in...");
      setTimeout(()=>signIn("credentials",{email:pendingEmail,password:signupPass,redirect:false}),800);
    } else {
      flash(setOtpMsg,"error","Invalid code.");
  ...
  const flushBatch = useCallback(async()=>{
    ...
    const snap=[...batchRef.current]; batchRef.current=[]; setPendingCount(0);
    const now=new Date();
    const fileName="log-"+now.toISOString().replace(/\.\d{3}Z$/,"").replace(/:/g,"-")+".json";
    setFlushLoading(true);
    try {
      const res=await fetch("/api/telemetry/trigger",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({entries:snap,fileName}),
      });
      if(res.ok){
        const d=await res.json();
        const newRec:LogRecord={
          recordId:d.recordId, fileName, fileHash:d.fileHash,
          timestamp:Date.now(), machineId:d.machineId??"MOTOR-001",
          entries:snap.length, onChain:d.onChain??false, txHash:d.txHash,
        };
        setLogRecords(p=>[newRec,...p]);
        setLastBatch(fileName.slice(0,22)+"...");
        showToast("🔗",`Stored! ${snap.length} entries${d.onChain?" → Celo Sepolia":" → local ledger"}`);
      } else { showToast("❌","Store failed — check server logs."); }
  ...
  async function refreshLogs(){
    setRefreshLoading(true); await fetchLogs(); setRefreshLoading(false);
  ...
  function startTelemetry(){
    entryTimer.current=setInterval(()=>{
      ...
      setLiveEntry(entry); setPendingCount(batchRef.current.length);
  ...
  useEffect(()=>{
    if(status==="authenticated"){startTelemetry();fetchLogs();}
  ...
  return (
    <>
          ...
          <div style={{width:"100%",maxWidth:460,background:"var(--card)",border:"1px solid var(--cborder)",borderRadius:24,padding:"2.5rem",backdropFilter:"blur(24px)",boxShadow:"0 0 0 1px rgba(59,130,246,.08),0 32px 64px rgba(3,7,30,.6)",animation:"cardIn .6s cubic-bezier(.16,1,.3,1) both"}}>
            ...
            {authTab==="signin"&&(
              <div>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.25rem"}}>
                  ...
                  <span onClick={()=>showToast("📧","Password reset sent! (Demo)")} style={{fontSize:".85rem",color:"var(--b400)",cursor:"pointer"}}>Forgot password?</span>
                </div>
            ...
            {authTab==="otp"&&(
              <div>
                ...
                <p style={{textAlign:"center",marginTop:"1rem",fontSize:".85rem",color:"var(--muted)"}}>Didn&apos;t receive it? <span onClick={()=>{const o=Math.floor(100000+Math.random()*900000).toString();setGenOTP(o);showToast("📧",`New code: ${o}`);}} style={{color:"var(--b400)",cursor:"pointer",fontWeight:500}}>Resend code</span></p>
        ...
        <div style={{position:"relative",zIndex:1,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
          ...
          <nav className="nb" style={{position:"sticky",top:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1rem 2rem",background:"rgba(3,7,30,.7)",backdropFilter:"blur(20px)",borderBottom:"1px solid var(--gborder)"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:"1rem"}}>
              <div style={{padding:".25rem .75rem",background:"rgba(6,182,212,.12)",border:"1px solid rgba(6,182,212,.25)",borderRadius:20,fontSize:".75rem",fontWeight:500,color:"var(--cyan)",display:"flex",alignItems:"center",gap:6}}>
                <div style={{width:6,height:6,borderRadius:"50%",background:"var(--cyan)",animation:"pulse 2s ease infinite"}}/>Celo Sepolia
              </div>
          ...
          {view==="dashboard"&&(
            <div className="db" style={{padding:"2rem",flex:1}}>
              ...
              <div className="mg" style={{display:"grid",gridTemplateColumns:"1fr 380px",gap:"1.5rem",marginBottom:"1.5rem"}}>
                ...
                <Card>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.25rem"}}>
                    <div><div style={{fontFamily:"'Syne',sans-serif",fontSize:"1rem",fontWeight:600}}>Live Telemetry Feed</div><div style={{fontSize:".78rem",color:"var(--muted)",marginTop:2}}>Real-time sensor readings · 3s interval</div></div>
                    <div style={{padding:".2rem .65rem",background:"rgba(16,185,129,.12)",border:"1px solid rgba(16,185,129,.25)",borderRadius:20,fontSize:".7rem",fontWeight:600,color:"var(--success)",display:"flex",alignItems:"center",gap:5,textTransform:"uppercase"}}>
                      <div style={{width:5,height:5,borderRadius:"50%",background:"var(--success)",animation:"pulse 1.5s ease infinite"}}/>Live
                    </div>
                  </div>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    ...
                    <span style={{fontSize:".75rem",color:"var(--muted)",fontFamily:"'DM Mono',monospace"}}>{liveEntry?new Date(liveEntry.timestamp).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",second:"2-digit"}):"--:--:--"}</span>
                  </div>
                ...
                <Card style={{flexDirection:"column",gap:"1rem"}}>
                  <div><div style={{fontFamily:"'Syne',sans-serif",fontSize:"1rem",fontWeight:600}}>Batch Status</div><div style={{fontSize:".78rem",color:"var(--muted)",marginTop:2}}>Next flush in</div></div>
                  ...
                  {[
                    ...
                    {icon:"🌐",bg:"rgba(6,182,212,.15)",label:"Network",val:"Celo Sepolia",col:"var(--cyan)"},
                  ].map(item=>(
                    <div key={item.label} style={{display:"flex",alignItems:"center",gap:12,padding:".85rem 1rem",background:"rgba(255,255,255,.03)",border:"1px solid var(--gborder)",borderRadius:12}}>
                      ...
                      <div><div style={{fontSize:".8rem",color:"var(--muted)"}}>{item.label}</div><div style={{fontFamily:"'DM Mono',monospace",fontSize:"1rem",fontWeight:500,marginTop:2,color:item.col??"var(--white)"}}>{item.val}</div></div>
                    </div>
                  ))}
                </Card>
              </div>
            ...
            <div className="db" style={{padding:"2rem",flex:1}}>
              <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:"1.5rem"}}>
                <div>
                  ...
                  <div style={{fontSize:".85rem",color:"var(--muted)",marginTop:".2rem"}}>{logRecords.length} records · Celo Sepolia + Local Ledger</div>
                </div>
              ...
              <div className="sg" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1rem",marginBottom:"1.5rem"}}>
                <SCard icon="🗂️" bg="rgba(59,130,246,.15)" label="Total Records" val={String(logRecords.length)} col="var(--b300)"><span style={{color:"var(--muted)"}}>All stored batches</span></SCard>
                <SCard icon="✅" bg="rgba(16,185,129,.15)" label="On-Chain" val={String(logRecords.filter(r=>r.onChain).length)} col="var(--success)"><span style={{color:"var(--muted)"}}>Submitted to Celo</span></SCard>
                <SCard icon="💾" bg="rgba(245,158,11,.15)" label="Local Only" val={String(logRecords.filter(r=>!r.onChain).length)} col="var(--warn)"><span style={{color:"var(--muted)"}}>Needs ENVs for chain</span></SCard>
              </div>
              ...
              <div className="mg" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1.5rem",alignItems:"start"}}>
                ...
                <Card style={{flexDirection:"column"}}>
                  ...
                  <div style={{marginTop:"1.25rem",display:"flex",gap:".75rem"}}>
                    ...
                    {verifyFile&&(
                      <button onClick={()=>{setVerifyFile(null);setVerifyResult(null);if(fileInputRef.current)fileInputRef.current.value="";}}
                        style={{flex:"0 0 auto",padding:".85rem 1.25rem",background:"rgba(255,255,255,.04)",border:"1px solid var(--gborder)",borderRadius:12,color:"var(--muted)",cursor:"pointer",fontSize:".9rem"}}>Clear</button>
                    )}
                  </div>
                  <div style={{marginTop:"1.25rem",padding:"1rem",background:"rgba(255,255,255,.02)",border:"1px solid var(--gborder)",borderRadius:12,fontSize:".8rem",color:"var(--muted)"}}>
                    <div style={{color:"var(--b300)",fontWeight:600,marginBottom:".5rem"}}>How it works</div>
                    The uploaded file is SHA-256 hashed using the same method as when it was stored. The hash is looked up in the local ledger. If the file has been modified even by one character, the hash will not match and tampering is detected.
                  </div>
                </Card>
                ...
                <Card style={{flexDirection:"column"}}>
                  ...
                  {!verifyResult&&!verifyLoading&&(
                    <div style={{textAlign:"center",padding:"3rem 1rem",color:"var(--muted)"}}>
                      ...
                      <div style={{fontSize:".9rem"}}>Upload a file and click Verify to check its integrity</div>
                    </div>
                  )}

                  {verifyLoading&&(
                    <div style={{textAlign:"center",padding:"3rem 1rem",color:"var(--muted)"}}>
                      <div style={{width:40,height:40,border:"3px solid rgba(59,130,246,.3)",borderTopColor:"#3b82f6",borderRadius:"50%",animation:"spin .7s linear infinite",margin:"0 auto 1rem"}}/>
                      <div style={{fontSize:".9rem"}}>Computing SHA-256 hash...</div>
                    </div>
                  )}
                  ...
                  {verifyResult&&!verifyLoading&&(
                    <div>
                      <div style={{padding:"1.25rem",borderRadius:14,marginBottom:"1.25rem",background:verifyResult.verified?"rgba(16,185,129,.1)":"rgba(239,68,68,.1)",border:`1px solid ${verifyResult.verified?"rgba(16,185,129,.3)":"rgba(239,68,68,.3)"}`}}>
                        ...
                        <div style={{fontFamily:"'Syne',sans-serif",fontSize:"1rem",fontWeight:700,color:verifyResult.verified?"var(--success)":"var(--danger)",marginBottom:".4rem"}}>{verifyResult.verified?"File Authentic":"File Tampered / Not Found"}</div>
                      ...
                      {verifyResult.verified&&(
                        <div style={{display:"flex",flexDirection:"column",gap:".5rem"}}>
                          {[
                            ...
                            {label:"On-Chain",val:verifyResult.onChain?"Yes ✅":"No (local ledger only)"},
                            ...(verifyResult.txHash?[{label:"Tx Hash",val:verifyResult.txHash.slice(0,18)+"..."}]:[]),
                          ].map(row=>(
          ...
          {view==="settings"&&(
            <div className="db" style={{padding:"2rem",flex:1}}>
              <div style={{marginBottom:"1.5rem"}}>
                ...
                <div style={{fontSize:".85rem",color:"var(--muted)",marginTop:".2rem"}}>Environment status and storage configuration</div>
              </div>
              <div className="mg" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1.5rem",alignItems:"start"}}>
                <Card style={{flexDirection:"column"}}>
                  ...
                  {[
                    ...
                    {key:"PRIVATE_KEY",hint:"Needed for on-chain storage",set:false},
                    ...
                    <div key={e.key} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:".75rem 1rem",background:"rgba(255,255,255,.02)",border:"1px solid var(--gborder)",borderRadius:10,marginBottom:".5rem"}}>
                      <div>
                        <div style={{fontSize:".85rem",fontWeight:500,fontFamily:"'DM Mono',monospace",color:"var(--b300)"}}>{e.key}</div>
                        <div style={{fontSize:".75rem",color:"var(--muted)",marginTop:2}}>{e.hint}</div>
                      </div>
                      <div style={{padding:".2rem .65rem",borderRadius:6,fontSize:".72rem",fontWeight:600,...(e.set?{background:"rgba(16,185,129,.12)",color:"var(--success)",border:"1px solid rgba(16,185,129,.25)"}:{background:"rgba(245,158,11,.12)",color:"var(--warn)",border:"1px solid rgba(245,158,11,.25)"})}}>{e.set?"SET":"FILL IN"}</div>
                    </div>
                  ))}
                </Card>
                <Card style={{flexDirection:"column",gap:"1rem"}}>
                  ...
                  <div style={{padding:"1.25rem",background:"rgba(59,130,246,.06)",border:"1px solid rgba(59,130,246,.2)",borderRadius:12}}>
                    <div style={{fontWeight:600,marginBottom:".5rem",display:"flex",alignItems:"center",gap:8}}>💾 Local JSON Ledger <span style={{padding:".15rem .5rem",background:"rgba(16,185,129,.15)",color:"var(--success)",borderRadius:6,fontSize:".7rem",border:"1px solid rgba(16,185,129,.25)"}}>ACTIVE</span></div>
                    <div style={{fontSize:".82rem",color:"var(--muted)",lineHeight:1.6}}>All batches are saved to <code style={{color:"var(--b300)"}}>data/ledger.json</code> in your project root. Works without any ENV variables and persists across restarts.</div>
                  </div>
                  <div style={{padding:"1.25rem",background:"rgba(245,158,11,.04)",border:"1px solid rgba(245,158,11,.15)",borderRadius:12}}>
                    <div style={{fontWeight:600,marginBottom:".5rem",display:"flex",alignItems:"center",gap:8}}>⛓ Celo Sepolia <span style={{padding:".15rem .5rem",background:"rgba(245,158,11,.12)",color:"var(--warn)",borderRadius:6,fontSize:".7rem",border:"1px solid rgba(245,158,11,.25)"}}>NEEDS ENVs</span></div>
                    <div style={{fontSize:".82rem",color:"var(--muted)",lineHeight:1.6}}>Fill <code style={{color:"var(--b300)"}}>PRIVATE_KEY</code> and <code style={{color:"var(--b300)"}}>CONTRACT_ADDRESS</code> in <code style={{color:"var(--b300)"}}>.env.local</code> to also submit to the blockchain. Records will be marked <strong>On-Chain ✅</strong>.</div>
                  </div>
      ...
      {/* TOAST */}
      <div style={{position:"fixed",bottom:"2rem",right:"2rem",zIndex:999,padding:".85rem 1.25rem",background:"rgba(10,31,107,.9)",border:"1px solid var(--cborder)",borderRadius:14,backdropFilter:"blur(20px)",display:"flex",alignItems:"center",gap:10,fontSize:".875rem",boxShadow:"0 16px 48px rgba(3,7,30,.6)",maxWidth:340,animation:toast.show?"slideUp .4s cubic-bezier(.16,1,.3,1) forwards":"slideDown .3s ease forwards",pointerEvents:toast.show?"auto":"none"}}>
...
function PBtn({onClick,loading,children}:{onClick:()=>void;loading:boolean;children:React.ReactNode}){
  return(
    <button onClick={onClick} disabled={loading} style={{width:"100%",padding:".85rem",background:"linear-gradient(135deg,var(--b600),var(--b500))",border:"none",borderRadius:12,color:"var(--white)",fontFamily:"'Syne',sans-serif",fontSize:".95rem",fontWeight:600,cursor:"pointer",boxShadow:"0 4px 16px rgba(37,99,235,.4)",display:"flex",alignItems:"center",justifyContent:"center",gap:8,opacity:loading?.8:1}}>
...
function Btn({onClick,loading,children}:{onClick:()=>void;loading:boolean;children:React.ReactNode}){
  return(
    <button onClick={onClick} disabled={loading} style={{padding:".5rem 1.25rem",background:"linear-gradient(135deg,var(--b600),var(--cyan2))",border:"none",borderRadius:10,color:"var(--white)",fontFamily:"'DM Sans',sans-serif",fontSize:".85rem",fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",gap:8,boxShadow:"0 2px 12px rgba(37,99,235,.35)",opacity:loading?.7:1}}>
  ...
  return(
    <div style={{background:"var(--card)",border:"1px solid var(--cborder)",borderRadius:18,padding:"1.5rem",backdropFilter:"blur(20px)"}}>
      ...
      <table className="ltable">
        ...
        <tbody>
          {records.length===0?(
            <tr><td colSpan={5} style={{textAlign:"center",color:"var(--muted)",padding:"2rem",fontSize:".85rem"}}>No records yet — batches appear here after the first flush (60s countdown or manual flush).</td></tr>
          ):records.map((r,i)=>(
            <tr key={r.recordId||i}>
